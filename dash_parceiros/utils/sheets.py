        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    PATH_CREDENCIAL_SHEETS_JSON, SCOPES)
    ...
    def get_planilha(self, pag_range):
        try:
            result = self.SPREADSHEETS.values().get(
                spreadsheetId=self.CODE_SHEETS, range=pag_range).execute()
            data = result['values']
            return data
        except HttpError as err:
            print(err)

    def upload_to_sheets(self, df: pd.DataFrame, pagina_sheets: str, chunk_size: int = 5000) -> None:
        ...
        # Upload em chunks para evitar erro 500 do Google Sheets API em DataFrames grandes
        set_with_dataframe(Worksheet, df.iloc[:chunk_size])
        for start in range(chunk_size, len(df), chunk_size):
            chunk = df.iloc[start:start + chunk_size]
            set_with_dataframe(Worksheet, chunk, row=start + 2, include_column_header=False)
